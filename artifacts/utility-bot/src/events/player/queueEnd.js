...
async function handleAutoplay(player, lastTrack, client) {
  ...
  let recommendations;
  try {
    recommendations = await fetchRecommendations(targetTrack, client);
  } catch (err) {
    // FIX: If Last.fm API key is missing or unconfigured, disable autoplay gracefully
    // instead of crashing the queue end handler
    if (err.message.includes('not configured') || err.message.includes('API key')) {
      logger.warn('QueueEnd', 'Autoplay disabled: Last.fm API key not set in .env (LASTFM_API_KEY)');
  ...
  // FIX: If Last.fm returns no recommendations, fall back to a YouTube search
  // for similar songs instead of throwing and killing autoplay entirely
  if (!recommendations?.length) {
    logger.warn('QueueEnd', `No Last.fm recommendations for "${lastTrack.info.title}", falling back to YouTube search`);
    try {
      const fallbackQuery = `${lastTrack.info.author} mix similar songs`;
      const fallbackResult = await client.music.search(fallbackQuery, { source: 'ytsearch' });
      if (fallbackResult?.tracks?.length > 0) {
        recommendations = fallbackResult.tracks.slice(0, 5).map(t => ({
          name: t.info.title,
          artist: t.info.author,
          trackInfo: t
        }));
        logger.info('QueueEnd', `Fallback found ${recommendations.length} tracks via YouTube search`);
      } else {
...
async function fetchRecommendations(track, client) {
  ...
  // FIX: Guard against missing Last.fm API key — throws a clear error instead of sending
  // an invalid request that returns a cryptic 400 from the Last.fm API
  if (!LASTFM_API_KEY) {
    throw new Error('Last.fm API key is not configured. Set LASTFM_API_KEY in your .env file.');
  }
