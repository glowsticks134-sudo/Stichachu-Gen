...
class RecommendationsCommand extends Command {
  ...
  async _fetchRecommendations(track, client) {
    ...
    for (let i   =0; i < Math.min(tracks.length, 6); i++) {
      ...
      let thumbnail   =config.assets?.defaultTrackArtwork || "https://cdn.discordapp.com/embed/avatars/0.png";
      let trackInfo   =null;
      ...
      try {
        ...
        if (searchResult?.tracks?.length > 0) {
          ...
          thumbnail   =trackInfo.info.artworkUrl || config.assets?.defaultTrackArtwork || "https://cdn.discordapp.com/embed/avatars/0.png";
        }
      } catch (searchError) {
  ...
  _buildRecommendationsContainer(currentTrack, recommendations, guildId, userId) {
    ...
    container.addSectionComponents(
      new SectionBuilder()
        ...
        .setThumbnailAccessory(new ThumbnailBuilder().setURL(currentTrack.info.artworkUrl || config.assets?.defaultTrackArtwork || "https://cdn.discordapp.com/embed/avatars/0.png"))
    );
